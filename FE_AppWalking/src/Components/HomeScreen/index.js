export { default as HomeRank } from './HomeRank';
export { default as ViewCoins } from './ViewCoin';
export { default as HomeEvent } from './HomeEvent';
export { default as HomeGetGift } from './HomeGetGift';
export { default as HomeButtonCoin } from './HomeButtonCoin';
export { default as HomePercentStep } from './HomePercentStep';
export { default as HomeButtonProfile } from './HomeButtonProFile';
export { default as HomeBoxStepHeader } from './HomeBoxStepRunner';
